...
function MyOrganisation() {
  const { getMyOrganisationInfo, addEmployee, addDepartment, addTitle, myOrganisationCalledOnce, myOrganisation } = useContext(AppContext);
  ...
  useEffect(async () => {
    if (myOrganisationCalledOnce.current) {
      return;
    } else {
      await getMyOrganisationInfo();
      myOrganisationCalledOnce.current = true;
    }
  }, []);
  ...
  return (
    <>
      {!myOrganisation ? (
        <LoadingButtonComponent loadSpinner={!myOrganisation} />
      ) : (
        <div>
